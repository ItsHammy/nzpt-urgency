...
def ensure_lbills_columns():
    """
    Make sure the `lbills` table has `mps` and `desc` columns.
    ...
    if "mps" not in cols:
        cursor.execute("ALTER TABLE lbills ADD COLUMN mps TEXT")
    if "desc" not in cols:
        cursor.execute("ALTER TABLE lbills ADD COLUMN desc TEXT")

    conn.commit()
    conn.close()


def get_lbills_needing_details():
    """
    Return list of (id, url) for legacy bills where mps or desc is NULL.
    """
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT id, url FROM lbills WHERE mps IS NULL OR desc IS NULL")
...
async def scrape_all_lbill_details():
    ensure_lbills_columns()
    bills = get_lbills_needing_details()

    if not bills:
        print("No legacy bills need details; all rows already have mps/desc.")
        return

    print(f"Found {len(bills)} legacy bills needing details.")
    ...
    async with async_playwright() as p:
        ...
        for bill_id, url in bills:
            print(f"Scraping details for legacy bill {bill_id} -> {url}")
            try:
    ...
    if updates:
        ...
        cursor = conn.cursor()
        cursor.executemany(
            "UPDATE lbills SET mps = ?, desc = ? WHERE id = ?",
            updates,
        )
        conn.commit()
        conn.close()
        print(f"Updated {len(updates)} legacy bills with mps/desc.")
    else:
        print("No updates to apply.")


if __name__ == "__main__":
    asyncio.run(scrape_all_lbill_details())